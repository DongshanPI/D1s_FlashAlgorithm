...
extern unsigned long cpu_intrpt_save(void);
void cpu_intrpt_restore(unsigned long cpsr);

/*critical operation    */
#define ENTER_CRITICAL(__cpsr)  do{__cpsr = cpu_intrpt_save();}while(0)
#define EXIT_CRITICAL(__cpsr)   do{cpu_intrpt_restore(__cpsr);}while(0)
...
/* -------------------------------- jiffies -----------------------------*/
extern long long aos_now(void);
#define HZ CONFIG_SYSTICK_HZ
#define jiffies ((unsigned long)aos_now())
