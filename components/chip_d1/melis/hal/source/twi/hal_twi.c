...
static int twi_stop(const unsigned long base_addr, int port)
{
    ...
    if (timeout == 0)
    {
        TWI_ERR("[twi%d] twi state(0x%0x) isn't idle(0xf8)",
    ...
    if (timeout == 0)
    {
        TWI_ERR("[twi%d] twi lcr(0x%0x) isn't idle(0x3a)",
...
err_out:
    twi->msgs_idx = code;
    twi->result = RESULT_ERR;
    TWI_ERR("packet transmission failed , status : 0x%0x", code);
...
#endif
	TWI_WARN("twi[%d] not support sys_config format \n", twi->port);
	return TWI_STATUS_ERROR;
}
...
twi_status_t hal_twi_init(twi_port_t port)
{
    ...
    {
        TWI_INFO("[twi%d] init ok..", twi->port);
        return TWI_STATUS_OK;
    }
...
errsem:
    ...
    TWI_ERR("[twi%d] init failed", twi->port);
    return TWI_STATUS_ERROR;
}
