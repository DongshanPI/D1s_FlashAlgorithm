...
static volatile uint16_t scan_status;
static volatile uint16_t connect_status = NET_CTRL_MSG_WLAN_DISCONNECTED;
...
int wifi_scan(aw_wifi_scan_results_t *scan_result,int max_num)
{
	...
	for (j = 0;j < TRY_FIND_SSID_TIME; j++) {
		...
		uint32_t end_time = XR_OS_GetTimeMS()+ scan_timeout_ms;
		scan_status = NET_CTRL_MSG_WLAN_SCAN_FAILED;
		WIFI_DEBUG("start time:(%lld)\r\n", XR_OS_GetTimeMS());
		while (!(scan_status == NET_CTRL_MSG_WLAN_SCAN_SUCCESS) &&
			(XR_OS_GetTimeMS() < end_time)) {
			XR_OS_MSleep(20);
		}

		if(XR_OS_GetTimeMS() >= end_time){
			WIFI_DEBUG("wlan sta scan timeout1 %d\n", scan_timeout_ms);
			free(results.ap);
			return -1;
		}
...
int get_wep_security(int *is_wep_security, char *ssid, int ssid_len){
	...
	for (j = 0;j < TRY_FIND_SSID_TIME; j++) {
		...
		if((XR_OS_GetTimeMS() >= end_time)){
			WIFI_DEBUG("wlan sta scan timeout2 %d\n", scan_timeout_ms);
			free(results.ap);
			return -1;
		}
...
int wifi_wep_connect(char *ssid, char *passwd)
{
	...
	uint32_t timeout = wep_open_connect_timeout_ms;
...
int wifi_connect(const char *ssid, const char *passwd)
{
	...
	if(wep_security == 1) {
		ret = wifi_wep_connect((char *)ssid, (char *)passwd);
	} else {
		ret= wifi_normal_connect((char *)ssid, (char *)passwd);
	}
...
int wifi_ap_stop(void)
{
	...
	// uint32_t timeout = (uint32_t)XR_OS_GetTimeMS() + link_dowm_timeout;
...
int wifi_get_ap_ssid(unsigned char *ssid)
{
    ...
    memcpy(ssid, apinfo.ssid.ssid, apinfo.ssid.ssid_len);
	ssid[apinfo.ssid.ssid_len] = 0;
    return 0;
}
...
static int wifi_set_sniff_sync_cfg(wlan_ext_sniff_sync_param_set_t *param)
{
	...
	ret = wlan_ext_request(g_wlan_netif, WLAN_EXT_CMD_SET_SNIFF_SYNC_CFG, (uint32_t)(uint64_t)(&g_sniff_sync_param));
...
int wifi_sta_recv_spec_frm(uint8_t enable, uint32_t channel, uint32_t frm_type)
{
	...
	if (enable) {
		...
		ret = wlan_ext_request(g_wlan_netif, WLAN_EXT_CMD_SET_FRM_FILTER, (uint32_t)(uint64_t)(&filter));
		/*
		...
		if (connect_status == NET_CTRL_MSG_WLAN_CONNECTED) {
			...
			ret = wlan_ext_request(g_wlan_netif, WLAN_EXT_CMD_SET_SNIFF_KP_ACTIVE, (uint32_t)(uint64_t)(&kp_act));
		}
	} else {
...
int wifi_sta_send_raw_frm(uint8_t *frm, int frm_len, uint32_t channel, uint32_t frm_type)
{
	...
	if (connect_status == NET_CTRL_MSG_WLAN_CONNECTED) {
		if (send_rawfrm_param.Enable == 0) {
			...
			wlan_ext_request(g_wlan_netif, WLAN_EXT_CMD_SET_SEND_RAW_FRM_CFG, (uint32_t)(uint64_t)(&send_rawfrm_param));
		}
