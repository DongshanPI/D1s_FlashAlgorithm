...
#endif
#define RTC_INFO(fmt, arg...) do {printf("\r\ninfo{"fmt"}\r\n", ##arg);}while(0)

#define RTC_ERR(fmt, arg...) do {printf("\r\nerr{"fmt"}\r\n", ##arg);}while(0)
...
static int rtc_mdelay(int ms)
{
    for (volatile int i = 0; i < 1*ms; i++) {
...
int hal_rtc_settime(struct rtc_time *rtc_tm)
{
    ...
    if (hal_rtc_wait(SUNXI_LOSC_CTRL,
                     SUNXI_LOSC_CTRL_RTC_HMS_ACC, 100))
    {
