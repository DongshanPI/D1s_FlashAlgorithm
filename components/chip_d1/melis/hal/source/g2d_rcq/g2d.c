...
int g2d_clk_init(__g2d_info_t *info)
{
	...
	if (ret)
		G2D_ERR_MSG("set clk:%ld's parent:%ld fail!\n", (unsigned long)info->clk[0], (unsigned long)info->clk_parent);
	return ret;
}
...
int g2d_clock_enable(__g2d_info_t *info)
{
	...
	if (ret) {
		G2D_ERR_MSG("Set clk rate fail:%ld:%u!\n", (unsigned long)info->clk[0], (unsigned int)info->clk_rate);
		goto OUT;
	}
