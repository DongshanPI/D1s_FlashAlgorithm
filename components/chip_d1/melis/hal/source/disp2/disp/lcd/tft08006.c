...
// #define TFT08006_DEBUG
#ifdef TFT08006_DEBUG
#define DBG_TFT(fmt, ...) do {printf("[TFT][%s %d]"fmt"\n", __func__, __LINE__, ##__VA_ARGS__);} while (0)
#else
#define DBG_TFT(fmt, ...) do {} while (0)
#endif
...
static void lcd_bl_open(u32 sel)
{
	...
	ret = sunxi_lcd_pwm_enable(sel);
    if (ret != 0) {
        DBG_TFT("lcd pwm enable failed.");
        return;
    }
    DBG_TFT("lcd pwm enable ok.");
}
...
static void lcd_panel_init(u32 sel)
{
    ...
    DBG_TFT("sel:%d", sel);
	ret = sunxi_lcd_dsi_clk_enable(sel);
    if (ret != 0) {
        DBG_TFT("dsi clk enable failed.");
        return;
    }
