...
static void start_work(void* work_data)
{
	int num_screens;
	int screen_id;
	int count = 0;
...
static int disp_init(void)
{
	...
	g_disp_work_queue = hal_workqueue_create("disp2", 8192, 21);
...
int disp_ioctl(int cmd, void *arg)
{
	...
	case DISP_GET_BKCOLOR:
		{
			struct disp_color para;

			if (mgr && (mgr->get_back_color != NULL)) {
                ret = mgr->get_back_color(mgr, &para);
                if (ret == 0) {
                    memcpy((void *)ubuffer[1], &para, sizeof(struct disp_color));
                }
            }
			break;
		}
