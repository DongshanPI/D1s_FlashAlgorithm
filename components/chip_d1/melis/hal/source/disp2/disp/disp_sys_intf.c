			  ...
			  u32 group_count_max)
{
	...
	data = gpio_list->data;
    DE_DBG2("----------gpio:%d, mul_sel:%d, pull:%d, drv_level:%d, data:%d", gpio, mul_sel, pull, drv_level, data);
	hal_gpio_pinmux_set_function(gpio, mul_sel);
	hal_gpio_set_direction(gpio, D1_GPIO_DIRECTION_OUTPUT);
...
int disp_sys_gpio_set_value(u32 p_handler, u32 value_to_gpio,
			    const char *gpio_name)
{
    DE_DBG2("pin: %d, data: %d\n", p_handler, value_to_gpio);
	if (p_handler)
...
int disp_sys_pin_set_state(char *dev_name, char *name)
{
    ...
    DE_DBG2("dev_name: %s, name: %s, len:%d\n", dev_name, name, len);
	if (!p_list || !len) {
