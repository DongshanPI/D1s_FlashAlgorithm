...
static int cmd_g2d_test_scale(int argc, const char **argv)
{
	...
	cpu_dcache_clean((unsigned long)buf1, IMG_SIZE1*3/2);
	cpu_dcache_clean((unsigned long)buf3, IMG_SIZE2*3/2);
	cpu_dcache_clean((unsigned long)buf5, IMG_SIZE3*3/2);

	//keep clean to avoid eviction.
	cpu_dcache_clean((unsigned long)buf2, IMG_SIZE1*4*3/2);
	cpu_dcache_clean((unsigned long)buf4, IMG_SIZE2/4*3/2);
	cpu_dcache_clean((unsigned long)buf6, IMG_SIZE3/16*3/2);
	...
	cpu_dcache_invalidate((unsigned long)buf2, IMG_SIZE1*4*3/2);
	cpu_dcache_invalidate((unsigned long)buf4, IMG_SIZE2/4*3/2);
	cpu_dcache_invalidate((unsigned long)buf6, IMG_SIZE3/16*3/2);
